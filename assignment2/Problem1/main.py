...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            imageio.imsave('images/grid_vis.png', grid_vis)

        # TODO (1): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            ray_vis = vis_rays(ray_bundle, image_size)
            imageio.imsave('images/ray_vis.png', ray_vis)
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            ret = 0.
            for i in tqdm.tqdm(range(0, pts.size(0), 64)):
                ret += render_points("images/point_vis.png", pts[i:(i+64),...])
            # np.save("images/point_vis_np", ret)
            ret[ret!=0] = 1 # or use np.clip
            plt.imsave("images/point_vis.png", ret)
