...
def get_pixels_from_image(image_size, camera):
    ...
    x = torch.arange(-1, 1, 2./W)
    y = torch.arange(-1, 1, 2./H)
...
def get_rays_from_pixels(xy_grid, image_size, camera: PerspectiveCameras):
    W, H = image_size[0], image_size[1]
    ...
    ndc_points = camera.unproject_points(ndc_points)

    # TODO (1): Get ray origins from camera center
    rays_o =  camera.get_camera_center()
    rays_o = camera.get_camera_center().repeat((len(ndc_points), 1))

    # TODO (1): Get normalized ray directions
    rays_d = F.normalize(ndc_points - rays_o, p=2., dim=-1)
    # Create and return RayBundlen
    return RayBundle(
        rays_o,
        rays_d,
