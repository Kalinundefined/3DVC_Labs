...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        transmittance = torch.exp(-rays_density * deltas + eps)
        transmittance = transmittance.cumprod(dim=1)
        transmittance[:, 1:] = transmittance[:, :-1]
        transmittance[:, 0] = 1
        # # TODO (4): Compute weight used for rendering from transmittance and density
        weights = transmittance * (1. - torch.exp(-rays_density * deltas + eps))
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            depth = self._aggregate(weights, density.view(-1, n_pts , 1)) # use zvals
